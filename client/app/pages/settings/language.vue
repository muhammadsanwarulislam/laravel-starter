<template>
    <div>
        ...
        <!-- Search and Filters -->
        <CommonSearchFilter :search-query="searchQuery" :filters="languageFilters" :filter-values="filters"
            :items-per-page="itemsPerPage" @search-change="handleSearch" @filter-change="handleFilterChange"
            @items-per-page-change="handleItemsPerPageChange" />

        <!-- Data Table -->
        <DataTable :data="data" :columns="languageColumns" :loading="isLoading" empty-message="No languages found.">
        ...
        <CommonPagination v-if="pagination && data.length > 0" :pagination="pagination" :items-per-page="itemsPerPage"
            @prev="prevPage" @next="nextPage" @page-change="goToPage"
            @items-per-page-change="handleItemsPerPageChange" />
...
import { languageFormConfig } from '~/config/forms/common';
