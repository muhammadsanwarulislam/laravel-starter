      ...
      <div class="mb-6">
        <div class="flex justify-center space-x-2 sm:space-x-3">
          <input v-for="(digit, index) in otpDigits" :key="index" v-model="otpDigits[index]" type="text"
            inputmode="numeric" pattern="[0-9]*" maxlength="1" @input="handleDigitInput($event, index)"
            @keydown="handleKeyDown($event, index)" @paste="handlePaste" @focus="focusedIndex = index"
            :ref="(el) => { if (el) digitInputs[index] = el }" :disabled="loading"
        ...
        <div class="mt-4 text-center">
          <p class="text-sm" :class="timer.isExpired ? 'text-red-500' : 'text-gray-600 dark:text-gray-400'">
            Code expires in
            <span class="font-semibold"
              :class="timer.isExpired ? 'text-red-500' : 'text-indigo-600 dark:text-indigo-400'">
              {{ timer.formatTime() }}
            </span>
          </p>
